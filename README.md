...
- ConvNeXt-модель **не рекомендуется** полноценно fine-tune'ить — разморозка слоёв плохо влияет на качество. В процессе оригинальной работы была сохранена лучшая модель. Если вы хотите воспроизвести опыт, для меньшей траты ресурсов вы можете убрать часть кода, отвечающую за разморозку слоёв модели при обучении
- Обучение моделей производилось на ноутбуке со следующими аппаратными характеристиками: GPU: Nvidia RTX 4050 laptop edition 6 GB VRAM, CPU: intel core i5-12500H, RAM 64 GB.
- Все скрипты с DINOv2 ожидают модели в папке `models/` с именами `fold_*_km.pth`.
- При проблемах с Selenium можно использовать иной способ получения адресов картинок
- Все папки указанные выше выставлены в файлах по умолчанию, как это запускалось на компьютере. Пожалуйста, отредактируйте пути в соответствии с тем, где вы располагаете файлы, чтобы не возникало ошибок
- Веса моделей представлены на https://huggingface.co/Infarondus/Klein-marchen_Base. 5 фолдов моделей klein-marchen используются для классификации средника, а klein-marchen_preprocess используется для определения переплёта
- Архитектурные решения и код были реализованы при помощи инструментов генеративного искусственного интеллекта Claude, Anthropic
